...
export default function AddCollateralPage() {
    ...
    const { data: walletAllowance, refetch: refetchAllowance } =
        useReadContract({
            address: pool?.collateralToken,
            abi: erc20Abi,
    ...
    const calculateUpdatedMetrics = () => {
        ...
        try {
            // Current values
            const currentEffectiveCollateral = Number(
                formatUnits(effectiveCollateral, 18)
            );
            const currentBaseCollateral = Number(
                formatUnits(baseCollateral, 18)
            );
            ...
            const newBaseCollateral =
                currentBaseCollateral + additionalCollateral;
            const newEffectiveCollateral =
                currentEffectiveCollateral +
                additionalCollateral * currentLeverage;
            ...
            const currentLiquidationPrice = Number(
                formatUnits(liquidationPrice || 0n, 18)
            );
    ...
    const handleApproveTokens = async () => {
        ...
        try {
            toast.loading('Approving collateral token...', {
                id: 'approve-tx',
            });
    ...
    const handleAddCollateral = async () => {
        if (!isClient || !collateralAmount || Number(collateralAmount) <= 0)
            return;
                ...
                <div className='bg-card rounded-lg border p-6 space-y-6'>
                    <div>
                        ...
                        <p className='text-sm text-muted-foreground'>
                            Add collateral to your {pool?.loanTokenSymbol}/
                            {pool?.collateralTokenSymbol} position to improve
                            health factor
                        </p>
                    </div>
                            ...
                            <div className='bg-muted/50 rounded-lg p-4 space-y-3'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    >
                                        ...
                                        {updatedMetrics.healthImprovement >
                                            0 && (
                                            <span className='text-green-500 ml-1'>
                                                (+
                                                {updatedMetrics.healthImprovement.toFixed(
                                                    2
                                                )}
                                                )
                                            </span>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    <span>
                                        $
                                        {updatedMetrics.newLiquidationPrice.toFixed(
                                            6
                                        )}
                                        {updatedMetrics.liquidationPriceReduction >
                                            0 && (
                                            <span className='text-green-500 ml-1'>
                                                (-
                                                {updatedMetrics.liquidationPriceReduction.toFixed(
                                                    6
                                                )}
                                                )
                                            </span>
                            ...
                            <div className='flex items-start gap-2'>
                                ...
                                <div>
                                    <p className='text-blue-700 dark:text-blue-300 text-sm'>
                                        Adding collateral improves your
                                        position's health factor and lowers the
                                        liquidation price, reducing the risk of
