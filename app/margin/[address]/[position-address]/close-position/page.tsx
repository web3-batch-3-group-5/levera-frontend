...
export default function ClosePositionPage() {
  ...
  const handleClosePosition = async () => {
    try {
      ...
      writeContract(
        ...
        {
          onSuccess: hash => {
            console.log('Close position transaction hash:', hash);
            setTxHash(hash);
  ...
  const buttonLabel = isWritePending || isConfirming ? 'Closing Position...' : 'Close Position';
        ...
        <div className='bg-card rounded-lg border p-6 space-y-6'>
          ...
          <div className='bg-muted/50 rounded-lg p-4 space-y-3'>
            ...
            <div className='flex justify-between'>
              ...
              <span>
                {formatTokenAmount(effectiveCollateral || 0n)} {pool.collateralTokenSymbol}
              </span>
            </div>
            <div className='flex justify-between'>
              <span className='text-sm text-muted-foreground'>Borrowed Amount</span>
              <span>
                {formatTokenAmount(borrowShares || 0n)} {pool.loanTokenSymbol}
              </span>
            </div>
            <div className='flex justify-between'>
          ...
          <div className='bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4'>
            ...
            <ul className='text-sm text-blue-700 dark:text-blue-400 space-y-2 list-disc pl-5'>
              ...
              <li>Your collateral will be returned to your wallet (minus the borrowed amount)</li>
          ...
          {isConfirmationVisible ? (
            <div className='space-y-4'>
              <div className='bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg p-4'>
                ...
                <p className='text-sm text-amber-700 dark:text-amber-400'>This action cannot be undone.</p>
              </div>

              <div className='flex gap-3'>
                <Button variant='destructive' className='flex-1' onClick={handleClosePosition} disabled={isDisabled}>
                  {buttonLabel}
                </Button>
                <Button
                  variant='outline'
