...
import { CreateLendingPoolParams, PoolDetails, PositionType } from '@/types';
...
export function useLendingPoolFactory() {
  ...
  useEffect(() => {
    ...
    try {
      const poolData: PoolDetails[] = poolAddresses.map((address, index) => {
        ...
        const collateralTokenUsdDataFeed = getResult<Address>(
          3,
        ...
        const isActive =
          poolStatusesData && poolStatusesData[index]?.status === 'success' && Boolean(poolStatusesData[index]?.result);
  ...
  const { data: tokenDetailsData } = useReadContracts({
    ...
    query: {
      ...
      enabled:
        poolBasicDetails.length > 0 &&
        poolBasicDetails.some(
          pool =>
            !!pool.loanToken &&
            pool.loanToken !== '0x0000000000000000000000000000000000000000' &&
            !!pool.collateralToken &&
            pool.collateralToken !== '0x0000000000000000000000000000000000000000',
        ),
    },
  });
  ...
  const createLendingPool = useCallback(
    async (params: CreateLendingPoolParams) => {
      try {
        const liquidationThreshold = params.liquidationThresholdPercentage;
