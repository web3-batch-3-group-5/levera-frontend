...
import type { PositionsResponse, PositionResponse } from '@/types/graphql';

export function usePoolPositions(
  poolAddress: string,
  { first = 10, skip = 0, orderBy = 'createdAt', orderDirection = 'desc' } = {},
) {
  return useQuery({
    queryKey: ['pool-positions', poolAddress, first, skip, orderBy, orderDirection],
    queryFn: () =>
      queryGraph<PositionsResponse>(GET_POOL_POSITIONS, {
        poolAddress,
        first,
        skip,
        orderBy,
...
export function useUserPositions(userAddress: string, { first = 10, skip = 0 } = {}) {
  return useQuery({
    queryKey: ['user-positions', userAddress, first, skip],
    queryFn: () =>
      queryGraph<PositionsResponse>(GET_USER_POSITIONS, {
        userAddress,
        first,
        skip,
      }),
...
export function usePosition(positionId: string) {
  return useQuery({
    ...
    queryFn: () =>
      queryGraph<PositionResponse>(GET_POSITION, {
        positionId,
      }),
    staleTime: CACHE_TIME.STANDARD,
