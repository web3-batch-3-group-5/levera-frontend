...
export function MarginCard({
    ...
    const {
        ...
        ltv,
        formattedValues,
        lendingPoolAddress: positionLendingPoolAddress,
        isLoading,
        error: positionError,
    ...
    lendingPoolAddress = lendingPoolAddress || positionLendingPoolAddress;
    ...
    // Debug logging
    useEffect(() => {
        console.log(`MarginCard for ${positionAddress}:`, {
            baseCollateral: baseCollateral?.toString(),
            lendingPoolAddress: lendingPoolAddress?.toString(),
            effectiveCollateral: effectiveCollateral?.toString(),
    ...
    const StatusBadge = () => (
        <div
            className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 ${healthColor.replace(
                'text',
                'bg'
            )}-100 ${healthColor} dark:${healthColor.replace(
                'text',
                'bg'
            )}-900/50`}
        >
            <span
                className={`size-1.5 rounded-full ${healthColor.replace(
                    'text',
                    'bg'
                )}`}
            ></span>
                    ...
                    <div className='flex gap-2'>
                        ...
                        <Button
                            onClick={(e) => {
                                e.stopPropagation();
                                router.push(
                                    `/margin/${lendingPoolAddress}/${positionAddress}`
                                );
                            }}
                            className='flex-1'
                            variant='default'
