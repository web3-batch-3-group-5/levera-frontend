...
export function PositionDashboard({
    ...
    const getPoolDetails = (lendingPoolAddress: Address) => {
        ...
        const poolIndex = poolAddresses.findIndex(
            (addr) => addr.toLowerCase() === lendingPoolAddress.toLowerCase()
        );
        ...
        <div className='bg-destructive/10 border border-destructive rounded-lg p-4 flex items-center gap-3'>
            ...
            <div>
                ...
                <p className='text-sm text-muted-foreground'>
                    {error?.message ||
                        'An error occurred while loading the positions. Please try refreshing.'}
                </p>
            </div>
            <Button
                variant='outline'
                ...
                <div className='space-y-4'>
                    {positionAddresses.map((positionAddress) => {
                        ...
                        const loanTokenSymbol =
                            poolData?.loanTokenSymbol || 'TOKEN';
                        const collateralTokenSymbol =
                            poolData?.collateralTokenSymbol || 'COLL';
